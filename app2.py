    ...
    try:
        ...
        try:
            ...
            if isinstance(plot_data, list):
                combined_data = {'data': [], 'layout': {'title': 'Combined Plot'}}
                for plot_json in plot_data:
                    plot = json.loads(plot_json)
                    combined_data['data'].extend(plot['data'])

                    if 'layout' in plot:
                        combined_data['layout'].update(plot['layout'])
                return jsonify(combined_data)
            else:
                return jsonify(plot_data)
        except json.JSONDecodeError as e:
            return jsonify({"error": "Failed to plot data as JSON.", "details": str(e)}), 500
    except Exception as e:
        return jsonify({"error": str(e)}), 500
